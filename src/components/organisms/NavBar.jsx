...
function NavLink({ to, children, active = false }) {
  return (
    <Link 
      to={to}
      className={`px-4 py-2 rounded-full text-sm font-montserrat font-medium transition-all duration-300 hover:bg-white/20 hover:shadow-md ${
        active 
          ? 'text-white bg-blue-main/80 shadow-md' 
...
function MobileNavLink({ to, children, onClick, active = false }) {
  return (
    <Link 
      to={to}
      onClick={onClick}
      className={`flex items-center gap-3 px-4 py-3 rounded-2xl text-base font-montserrat font-medium transition-all duration-300 ${
        active 
          ? 'text-white bg-blue-main/90 shadow-md' 
          : 'text-blue-dark hover:text-white hover:bg-blue-main/60'
      }`}
    >
      <div className={`w-2 h-2 rounded-full transition-all duration-300 ${
        active ? 'bg-white' : 'bg-blue-main/60'
      }`}></div>
      {children}
    </Link>
  );
